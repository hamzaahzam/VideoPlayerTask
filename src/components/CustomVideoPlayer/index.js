...
export class CustomVideoPlayer extends Component {
    ...
    componentDidMount() {
        ...
        if(this.props.selectedVideo.playBackPosition && this.player) {
            this.player.seek(this.props.selectedVideo.playBackPosition);
        }
    }
    ...
    startCountdown() {
        this.timerInterval = setInterval(() => {
            if(this.state.nextVideoTimer > 0) {
                this.setState((prevState) => ({ nextVideoTimer: prevState.nextVideoTimer - 1 }));
            } else {
                // Countdown has finished; you can perform actions here
                clearInterval(this.timerInterval); // Stop the countdown
                this.setState({ nextVideoTimer: 5 });
                if(this.props.startNextVideo) {
                    this.props.startNextVideo();
                }
            }
    ...
    jumpTenSec(e) {
        this.player && this.player.seek(this.state.videoCurrentTime + e);
    }
        ...
        return (
            <View style={ [styles.controls, customStyles.controls] }>
                ...
                <View style={ { flexDirection: "row" } }>
                    <TouchableOpacity
                        onPress={ () => this.jumpTenSec(-10) }
                    >
                        <Icon
                            style={ [styles.playControl, customStyles.controlIcon, customStyles.playIcon, { transform: [{ scaleX: -1 }] }] }
                            name={ 'forward-10' }
                            size={ 32 }
                        />
                    </TouchableOpacity>
        ...
        return (
            <View style={ customStyles.videoWrapper }>
                ...
                >
                    ...
                    />
                    { this.state.nextVideoTimer != 5 && <View style={ styles.nextVideoContainer }>
                        <Text style={ styles.nextVideoText }>{ `Next video in ${this.state.nextVideoTimer}` }</Text>
                    </View> }
                </View>
